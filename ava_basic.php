<?php
/*
	Plugin Name: AVA Basic Video Gallery, wpvideoplugins.com
	Plugin URI: http://www.wpvideoplugins.com/ava-basic-video/
	Description: With this plugin you can create video galleries with flv videos and/or youtube videos. You can use the plugin as a widget. USE: Install and activate the plugin. You will see a new button on your wordpress administrator, "ava Basic." Click here to create your videos galleries. To insert a gallery in your posts, type [ava_basic X/], where X is the ID of the gallery.
	Version: 1.3
...
function ava_basic_head() {
	
	$site_url = get_option( 'siteurl' );
			
			
}
function ava_basic($content){
	$content = preg_replace_callback("/\[ava_basic ([^]]*)\/\]/i", "ava_basic_render", $content);
	return $content;
	
}

function ava_basic_render($tag_string){
$contador=rand(9, 9999999);
	$site_url = get_option( 'siteurl' );
global $wpdb; 	
$table_name = $wpdb->prefix . "ava_basic";	
	...
	if($auxivideo!="") {
		...
		if(strstr($auxivideo, "http")) {
			if(strpos($auxivideo, "youtube")>0) {
				$auxivideo=getYTidbasicava($auxivideo);
				$auxtipo=2;
...
$texto.='&cantidad='.$cont1;
  ...
  <param name="movie" value="'.$site_url.'/wp-content/plugins/ava-basic-video-gallery/ava_basic.swf" />
  ...
  <param name="expressinstall" value="'.$site_url.'/wp-content/plugins/ava-basic-video-gallery/Scripts/expressInstall.swf" />
  <!-- Next object tag is for non-IE browsers. So hide it from IE using IECC. -->
  <!--[if !IE]>-->
  <object type="application/x-shockwave-flash" data="'.$site_url.'/wp-content/plugins/ava-basic-video-gallery/ava_basic.swf" width="'.$width.'" height="'.$height.'">
    <!--<![endif]-->
    <param name="quality" value="high" />
    <param name="wmode" value="transparent" />
    	<param name="flashvars" value="'.$texto.'" />
...
function getYTidbasicava($ytURL) {
#
...
function ava_basic_desinstala(){
	global $wpdb; 
	$table_name = $wpdb->prefix . "ava_basic";
	$sql = "DROP TABLE $table_name";
	$wpdb->query($sql);
}	
function ava_basic_panel(){
	global $wpdb; 
	$table_name = $wpdb->prefix . "ava_basic";	
...
function widget_ava_basic($args) {

 
  
    extract($args);
	
	  $options = get_option("widget_ava_basic");
  if (!is_array( $options ))
{
$options = array(
      'title' => 'Ava Basic',
...
$options = array(
  ...
  if ($_POST['ava-Submit'])
  {
    ...
    update_option("widget_ava_basic", $options);
  }
...
if(empty($myrows)) {
	...
	echo '
	<p>First create a new gallery of videos, from the administration of BASIC AVA plugin.</p>
	';
}
...
function ava_basic_init(){
	register_sidebar_widget(__('Ava Basic'), 'widget_ava_basic');
	register_widget_control(   'Ava Basic', 'ava_basic_control', 300, 300 );
}


function ava_basic_add_menu(){	
	if (function_exists('add_options_page')) {
		//add_menu_page
		add_menu_page('ava_basic', 'ava Basic', 8, basename(__FILE__), 'ava_basic_panel');
	}
}
if (function_exists('add_action')) {
	add_action('admin_menu', 'ava_basic_add_menu'); 
}
add_action('wp_head', 'ava_basic_head');
add_filter('the_content', 'ava_basic');
add_action('activate_ava_basic/ava_basic.php','ava_basic_instala');
add_action('deactivate_ava_basic/ava_basic.php', 'ava_basic_desinstala');
add_action("plugins_loaded", "ava_basic_init");
?>